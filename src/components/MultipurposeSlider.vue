<template>
    <div class="slider-container">
      ...
      <div class="slider-wrapper">
        <input
          type="range"
          :min="isArray ? 0 : min"
          :max="isArray ? values.length - 1 : max"
          :step="1"
  ...
  export default {
    ...
    data() {
      return {
        currentIndex: 0, // Tracks the current slider index or value
      };
    },
    computed: {
      isArray() {
  ...
  .current-value {
    position: absolute;
    top: 35px; /* Adjusted to place the value display below the slider */
    font-size: 14px;
    padding: 5px 10px;
    background-color: #3498db;
    color: white;
    border-radius: 5px;
    transition: left 0.3s ease; /* Smooth transition when slider moves */
    white-space: nowrap;
    pointer-events: none; /* Prevent it from interfering with slider events */
  }
  </style>
